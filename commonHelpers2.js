import{o as z,a as U,i as b}from"./assets/modals-functions-7f0c1471.js";import{a as c}from"./assets/vendor-8cce9181.js";const P=/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/,C="feedback-form-state",g=document.querySelector(".footer-subscription"),N=g.querySelector(".input-footer"),D=document.querySelector(".footer-button"),_=async e=>{try{return(await c.post("https://energyflow.b.goit.study/api/subscription",{email:e})).data}catch(t){throw new Error(t.message)}},F=async e=>{e.preventDefault(),D.blur();const t=N.value.trim();if(!P.test(t))z("Enter a valid email!");else{try{await _(t),U("Successful subscription!")}catch{z("There is already a user with this email!")}localStorage.removeItem(C),g.reset()}};g.addEventListener("submit",F);g.addEventListener("input",e=>{const t=N.value.trim(),n=JSON.stringify({footerEmail:t});localStorage.setItem(C,n)});c.defaults.baseURL="https://energyflow.b.goit.study/api";const E=document.querySelector(".exersizes-cards-container"),o=document.querySelector(".exersizes-cards-container-wrapper"),k=document.querySelector(".exersizes-result-card-container"),m=document.querySelector(".exersizes-result-card-container-wrapper"),d=document.querySelector(".exersizes-message-container"),J=document.querySelector(".exersizes-pagination-container"),I=document.querySelector(".exersizes-input-container"),a=document.querySelector(".exersizes-input-btn"),j=document.querySelector(".exersizes-input-btn-s"),T=document.querySelector(".dash"),p=document.querySelector(".exercise-name"),$=1,W=document.querySelector(".exersizes-list");document.querySelector(".exersizes-pagination-list");document.querySelector(".exersizes-pagination-btn");K();document.addEventListener("DOMContentLoaded",O());W.addEventListener("click",e=>{if(e.preventDefault(),h(),e.target.nodeName==="BUTTON"){const t=e.target.textContent.trim();sessionStorage.clear(),sessionStorage.setItem("filterType",JSON.stringify(t)),O(t),Q(),ne(),X(e),v()}});o.addEventListener("click",B);function B(e){if(e.preventDefault(),h(),e.target.nodeName!=="DIV"&&e.target.nodeName!=="H3"&&e.target.nodeName!=="P")return;const t=e.target.dataset.filter,n=e.target.dataset.target;A(t,n,$),Y(e),ee(),v(),sessionStorage.setItem("filterSubType",JSON.stringify(n)),sessionStorage.setItem("filterType",JSON.stringify(t))}J.addEventListener("click",e=>{if(e.preventDefault(),h(),e.target.nodeName==="BUTTON"){const t=JSON.parse(sessionStorage.getItem("filterType")),n=e.target.textContent.trim();let r;sessionStorage.getItem("filterSubType")&&(r=JSON.parse(sessionStorage.getItem("filterSubType"))),re(),V(t,r,n),Z(e),v()}});async function O(e,t,n){const r=await c.get("/filters",{params:y(e,t,n)});try{if(r.data.results.length===0)throw new Error("No results found...");e=r.data.results[0].filter,M(r),w(r)}catch{f()}}async function A(e,t,n){const r=await c.get("/exercises",{params:y(e,t,n)});try{if(r.data.results.length===0)throw new Error("No results found...");H(r),n===1&&w(r)}catch{f()}}async function G(e,t,n){const r=await c.get("/filters",{params:y(e,t,n)});try{if(r.data.results.length===0)throw new Error("No results found...");e=r.data.results[0].filter,M(r)}catch{f()}}async function V(e,t,n){t?await A(e,t,n):await G(e,t,n)}async function S(e){if(e.target.nodeName!=="BUTTON"&&e.keyCode!==13)return;h();const t=document.querySelector(".exersizes-input").value.trim().toLowerCase(),n=JSON.parse(sessionStorage.getItem("filterType")),r=JSON.parse(sessionStorage.getItem("filterSubType"));try{if(t.length!==0){const s=await c.get("/exercises",{params:y(n,r,$,t)});if(s.data.results.length===0)throw new Error("No results found...");H(s),w(s)}}catch{f()}l(),v()}async function M(e){const t=e.data.results;o.classList.remove("hidden"),m.classList.add("hidden"),d.classList.add("hidden"),E.innerHTML="";const n=t.map(r=>{const s=r.filter,i=r.name;return`<li class="first-filter"><div class="exersizes-card-bytype" tabindex="0" style="background: linear-gradient(
    0deg,
    rgba(16, 16, 16, 0.7) 0%,
