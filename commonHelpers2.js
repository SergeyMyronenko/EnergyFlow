import"./assets/style-89757184.js";import{S as z,a as o}from"./assets/vendor-20b07447.js";const H=document.querySelector(".header-burger"),M=document.querySelector(".mobile-menu-wrapper"),R=document.querySelector(".mobile-menu-close-btn");function U(){M.classList.remove("visually-hidden")}function P(){M.classList.add("visually-hidden")}H.addEventListener("click",U);R.addEventListener("click",P);const _=/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/,k="feedback-form-state",g=document.querySelector(".footer-subscription"),C=g.querySelector(".input-footer"),F=document.querySelector(".footer-button");function E(e){z.fire({position:"center",icon:"error",title:"Enter a valid email!",showConfirmButton:!1,timer:2e3})}function J(e){z.fire({position:"center",icon:"success",title:"Congratulations, you have successfully subscribed!",showConfirmButton:!1,timer:2e3})}const j=async e=>{try{return(await o.post("https://energyflow.b.goit.study/api/subscription",{email:e})).data}catch(t){throw new Error(t.message)}},Q=async e=>{e.preventDefault(),F.blur();const t=C.value.trim();if(!_.test(t))E();else{try{await j(t),J("Successful subscription!")}catch(s){E("Error subscribing: "+s.message)}localStorage.removeItem(k),g.reset()}};g.addEventListener("submit",Q);g.addEventListener("input",e=>{const t=C.value.trim(),s=JSON.stringify({footerEmail:t});localStorage.setItem(k,s)});function T(e){return o.get(`https://energyflow.b.goit.study/api/exercises/${e}`)}function V(e){return`
  ...
  `}const r={ratingModal:document.querySelector(".js-backdrop-modal"),closeBtn:document.querySelector(".js-rating-close"),form:document.querySelector(".js-rating-form"),rateStars:document.querySelector(".js-stars-list"),star:document.querySelectorAll(".js-rating-star"),openModalsBtn:document.querySelector(".exersizes-cards-container"),openRatingBtn:document.querySelector(".modal-button-rating"),exerciseModal:document.querySelector(".modal"),closeExerciseBtn:document.querySelector(".modal-button-close"),exsCont:document.querySelector(".exs-container")};document.addEventListener("click",W);document.addEventListener("click",G);document.addEventListener("click",Y);document.addEventListener("keydown",K);r.closeBtn.addEventListener("click",X);r.closeExerciseBtn.addEventListener("click",Z);async function G(e){try{if(e.target.classList.contains("exersizes-card-btn")){const t=await T(e.target.dataset.id);r.exsCont.innerHTML=V(t.data),r.exerciseModal.classList.toggle("is-open");const s=document.querySelector(".ex-rating-active"),i=document.querySelector(".modal-rating-value");s.style.width=await`${parseInt(i.textContent)/.05}%`}}catch(t){throw new Error(t.message)}}async function W(e){e.target.classList.contains("modal-button-rating")&&r.ratingModal.classList.toggle("is-open")}function Y(e){if(e.target.classList.contains("backdrop")&&r.ratingModal.classList.contains("is-open")){r.ratingModal.classList.remove("is-open");return}e.target.classList.contains("backdrop")&&r.exerciseModal.classList.contains("is-open")&&r.exerciseModal.classList.remove("is-open")}function X(e){e.currentTarget&&r.ratingModal.classList.remove("is-open")}function Z(e){e.currentTarget&&r.exerciseModal.classList.remove("is-open")}function K(e){if(e.code==="Escape"&&r.ratingModal.classList.contains("is-open")){r.ratingModal.classList.remove("is-open");return}e.code==="Escape"&&r.exerciseModal.classList.contains("is-open")&&r.exerciseModal.classList.remove("is-open")}const c={modalBackdrop:document.querySelector(".js-backdrop-modal"),form:document.querySelector(".js-rating-form"),exerciseModal:document.querySelector(".modal"),rateValue:document.querySelector(".js-rating-data"),stars:document.querySelectorAll(".rating-label")},ee="64f389465ae26083f39b17a4";c.form.addEventListener("submit",I);c.stars.forEach(e=>{e.addEventListener("click",B)});function B(e){c.rateValue.textContent=`${e.target.dataset.rate}.0`}async function I(e){e.preventDefault();try{const t=await o.patch(`https://energyflow.b.goit.study/api/exercises/${ee}/rating`,{rate:parseInt(e.target.elements.rating.value),email:e.target.elements.email.value.trim(),review:e.target.elements.comment.value.trim()})}catch(t){throw new Error(t.message)}finally{c.modalBackdrop.classList.remove("is-open"),c.exerciseModal.classList.remove("is-open"),e.target.reset(),c.form.removeEventListener("submit",I),c.stars.forEach(t=>{t.removeEventListener("click",B)})}}async function te(){const e=localStorage&&JSON.parse(localStorage.getItem("quoteData")),t=new Date().getDate();return e&&e.timestamp===t?e:se()}async function se(){try{const e=await o.get("https://energyflow.b.goit.study/api/quote"),t={quote:e.data.quote,author:e.data.author,timestamp:new Date().getDate()};return localStorage.setItem("quoteData",JSON.stringify(t)),t}catch(e){console.error("Помилка:",e)}}async function ae(e){try{const t=await te(),s=document.querySelector(e.quoteTextSelector),i=document.querySelector(e.quoteAuthorSelector);if(!s||!i)throw new Error("Елементи для відображення цитати не знайдено");s.innerText=t.quote,i.innerText=t.author}catch(t){console.error("Сталася помилка при відображенні цитати:",t)}}const h={quoteTextSelector:".quote-text",quoteAuthorSelector:".quote-author"};document.querySelector(h.quoteTextSelector)&&document.querySelector(h.quoteAuthorSelector)&&ae(h);const ie=document.querySelector(".modal");document.addEventListener("click",ne);let N="favoriteData";const x=JSON.parse(localStorage.getItem(N))||[];async function ne(e){try{if(e.target.classList.contains("modal-button-favorites")){const s=(await T(e.target.dataset.id)).data;if(!x.some(a=>a._id===s._id))x.push(s);else return;localStorage.setItem(N,JSON.stringify(x)),ie.classList.remove("is-open")}}catch(t){console.log(t.message)}}o.defaults.baseURL="https://energyflow.b.goit.study/api";const l=document.querySelector(".exersizes-cards-container"),d=document.querySelector(".exersizes-result-card-container"),u=document.querySelector(".exersizes-message-container"),re=document.querySelector(".exersizes-pagination-container"),$=document.querySelector(".dash"),p=document.querySelector(".exercise-name"),A=1,oe=document.querySelector(".exersizes-list");document.querySelector(".exersizes-pagination-list");document.querySelector(".exersizes-pagination-btn");document.addEventListener("DOMContentLoaded",S());oe.addEventListener("click",e=>{if(e.preventDefault(),f(),e.target.nodeName==="BUTTON"){const t=e.target.textContent.trim();sessionStorage.clear(),sessionStorage.setItem("filterType",JSON.stringify(t)),S(t),me(),he(),de(e),y()}});l.addEventListener("click",e=>{if(e.preventDefault(),f(),e.target.nodeName!=="DIV"&&e.target.nodeName!=="H3"&&e.target.nodeName!=="P")return;const t=e.target.dataset.filter,s=e.target.dataset.target;D(t,s,A),ue(e),fe(),y(),sessionStorage.setItem("filterSubType",JSON.stringify(s)),sessionStorage.setItem("filterType",JSON.stringify(t))});re.addEventListener("click",e=>{if(e.preventDefault(),f(),e.target.nodeName==="BUTTON"){const t=sessionStorage.getItem("filterType").slice(1,-1),s=e.target.textContent.trim();let i;sessionStorage.getItem("filterSubType")?i=sessionStorage.getItem("filterSubType").slice(1,-1):i=sessionStorage.getItem("filterSubType"),ce(t,i,s),pe(e),y()}});async function S(e,t,s){const i=await o.get("/filters",{params:q(e,t,s)});try{if(i.data.results.length===0)throw new Error("No results found...");e=i.data.results[0].filter,le(i),w(i)}catch{L()}}async function D(e,t,s){const i=await o.get("/exercises",{params:q(e,t,s)});try{if(i.data.results.length===0)throw new Error("No results found...");O(i),s===1&&w(i)}catch{L()}}async function ce(e,t,s){t?await D(e,t,s):await S(e,t,s)}async function b(e){if(e.target.nodeName!=="BUTTON"&&e.keyCode!==13)return;f();const t=document.querySelector(".exersizes-input").value.trim().toLowerCase(),s=sessionStorage.getItem("filterType").slice(1,-1),i=sessionStorage.getItem("filterSubType").slice(1,-1);try{if(t.length===0)throw new Error("Nthing to search...");{const a=await o.get("/exercises",{params:q(s,i,A,t)});if(a.data.results.length===0)throw new Error("No results found...");m(),O(a),w(a),y()}}catch(a){console.log(a),L()}}async function le(e){const t=e.data.results;l.classList.remove("visually-hidden"),d.classList.add("visually-hidden"),u.classList.add("visually-hidden"),l.innerHTML="";const s=t.map(i=>{const a=i.filter,n=i.name;return`<li class="first-filter"><div class="exersizes-card-bytype" tabindex="0" style="background: linear-gradient(
    0deg,
    rgba(16, 16, 16, 0.7) 0%,
