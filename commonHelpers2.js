import{i as D}from"./assets/modals-functions-a3f6c7fe.js";import{i as C,a as d}from"./assets/vendor-8cce9181.js";function q(e="Operation has been success"){C.success({message:e,messageColor:"#fff",backgroundColor:"rgba(27, 27, 27, 0.60)",position:"topRight",theme:"dark",closeOnClick:"true",messageSize:"16",messageLineHeight:"16"})}function z(e="Sorry... something wrong happened"){C.error({message:e,messageColor:"#fff",backgroundColor:"rgba(27, 27, 27, 0.60)",position:"topRight",theme:"dark",closeOnClick:"true",messageSize:"16",messageLineHeight:"16"})}const U=/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/,N="feedback-form-state",f=document.querySelector(".footer-subscription"),T=f.querySelector(".input-footer"),j=document.querySelector(".footer-button"),P=async e=>{try{return(await d.post("https://energyflow.b.goit.study/api/subscription",{email:e})).data}catch(t){throw new Error(t.message)}},_=async e=>{e.preventDefault(),j.blur();const t=T.value.trim();if(!U.test(t))z("Enter a valid email!");else{try{await P(t),q("Successful subscription!")}catch{z("There is already a user with this email!")}localStorage.removeItem(N),f.reset()}};f.addEventListener("submit",_);f.addEventListener("input",e=>{const t=T.value.trim(),r=JSON.stringify({footerEmail:t});localStorage.setItem(N,r)});const o={modalBackdrop:document.querySelector(".js-backdrop-modal"),form:document.querySelector(".js-rating-form"),exerciseModal:document.querySelector(".modal"),rateValue:document.querySelector(".js-rating-data"),starsContainer:document.querySelector(".js-stars-list")};o.form.addEventListener("submit",J);o.starsContainer.addEventListener("click",F);function F(e){e.target.classList.contains("rating-label")&&(o.rateValue.textContent=`${e.target.dataset.rate}.0`)}async function J(e){e.preventDefault();try{const t=await d.patch(`https://energyflow.b.goit.study/api/exercises/${D}/rating`,{rate:parseInt(e.target.elements.rating.value),email:e.target.elements.email.value.trim(),review:e.target.elements.comment.value.trim()});q()}catch(t){throw new Error(t.message)}finally{W()}}function W(){o.modalBackdrop.classList.remove("is-open"),o.exerciseModal.classList.remove("is-open"),o.rateValue.textContent="0.0",o.form.reset()}d.defaults.baseURL="https://energyflow.b.goit.study/api";const E=document.querySelector(".exersizes-cards-container"),c=document.querySelector(".exersizes-cards-container-wrapper"),k=document.querySelector(".exersizes-result-card-container"),g=document.querySelector(".exersizes-result-card-container-wrapper"),l=document.querySelector(".exersizes-message-container"),G=document.querySelector(".exersizes-pagination-container"),I=document.querySelector(".exersizes-input-container"),a=document.querySelector(".exersizes-input-btn"),V=document.querySelector(".exersizes-input-btn-s"),B=document.querySelector(".dash"),p=document.querySelector(".exercise-name"),M=1,X=document.querySelector(".exersizes-list");document.querySelector(".exersizes-pagination-list");document.querySelector(".exersizes-pagination-btn");se();document.addEventListener("DOMContentLoaded",$());X.addEventListener("click",e=>{if(e.preventDefault(),v(),e.target.nodeName==="BUTTON"){const t=e.target.textContent.trim();sessionStorage.clear(),sessionStorage.setItem("filterType",JSON.stringify(t)),$(t),ee(),ae(),Z(e),x()}});c.addEventListener("click",O);function O(e){if(e.preventDefault(),v(),e.target.nodeName!=="DIV"&&e.target.nodeName!=="H3"&&e.target.nodeName!=="P")return;const t=e.target.dataset.filter,r=e.target.dataset.target;A(t,r,M),K(e),re(),x(),sessionStorage.setItem("filterSubType",JSON.stringify(r)),sessionStorage.setItem("filterType",JSON.stringify(t))}G.addEventListener("click",e=>{if(e.preventDefault(),v(),e.target.nodeName==="BUTTON"){const t=JSON.parse(sessionStorage.getItem("filterType")),r=e.target.textContent.trim();let n;sessionStorage.getItem("filterSubType")&&(n=JSON.parse(sessionStorage.getItem("filterSubType"))),oe(),Q(t,n,r),te(e),x()}});async function $(e,t,r){const n=await d.get("/filters",{params:y(e,t,r)});try{if(n.data.results.length===0)throw new Error("No results found...");e=n.data.results[0].filter,H(n),w(n)}catch{h()}}async function A(e,t,r){const n=await d.get("/exercises",{params:y(e,t,r)});try{if(n.data.results.length===0)throw new Error("No results found...");R(n),r===1&&w(n)}catch{h()}}async function Y(e,t,r){const n=await d.get("/filters",{params:y(e,t,r)});try{if(n.data.results.length===0)throw new Error("No results found...");e=n.data.results[0].filter,H(n)}catch{h()}}async function Q(e,t,r){t?await A(e,t,r):await Y(e,t,r)}async function S(e){if(e.target.nodeName!=="BUTTON"&&e.keyCode!==13)return;v();const t=document.querySelector(".exersizes-input").value.trim().toLowerCase(),r=JSON.parse(sessionStorage.getItem("filterType")),n=JSON.parse(sessionStorage.getItem("filterSubType"));try{if(t.length!==0){const s=await d.get("/exercises",{params:y(r,n,M,t)});if(s.data.results.length===0)throw new Error("No results found...");u(),R(s),w(s)}}catch{h()}x()}async function H(e){const t=e.data.results;c.classList.remove("hidden"),g.classList.add("hidden"),l.classList.add("hidden"),E.innerHTML="";const r=t.map(n=>{const s=n.filter,i=n.name;return`<li class="first-filter"><div class="exersizes-card-bytype" tabindex="0" style="background: linear-gradient(
    0deg,
    rgba(16, 16, 16, 0.7) 0%,
    rgba(16, 16, 16, 0.7) 100%
      ...
      </div></li>`}).join("");E.insertAdjacentHTML("beforeend",r),c.addEventListener("keyup",m)}function m(e){e.keyCode===13&&O(e)}function R(e){const t=e.data.results;c.removeEventListener("keyup",m),g.classList.remove("hidden"),c.classList.add("hidden"),l.classList.add("hidden"),c.removeEventListener("keyup",m),k.innerHTML="";let r;const n=t.map(s=>{let i=s.name,b=s.target;r=s._id;const L=window.innerWidth;return L>=1440?(i.length>25&&(i=s.name[0].toUpperCase()+s.name.slice(1,25).trim()+"..."),b.length>=9&&(b=s.target[0].toUpperCase()+s.target.slice(1,8).trim()+"...")):L<1440&&L>=768?i.length>17&&(i=s.name[0].toUpperCase()+s.name.slice(1,16).trim()+"..."):(i=s.name[0].toUpperCase()+s.name.slice(1,20).trim()+"...",console.log("320")),`        <li class="second-filter" aria-label="Exercise"><div class="exersizes-card" tabindex="0">
...
<svg class="exersizes-card-title-icon" width="24" height="24" aria-label="Runner icon">
    ...
    <ul class="exersizes-card-info-list">
        ...
        <li class="exersizes-card-info-item"><p class="exersizes-card-info-descr" aria-label="Exercise name">Target:
            <span class="exersizes-card-info-data" data-filter-sub-type>${b}</span></p></li>
    </ul>
