import{o as w,a as R,i as L}from"./assets/modals-functions-4df62e74.js";import{a as d}from"./assets/vendor-8cce9181.js";const D=/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/,q="feedback-form-state",f=document.querySelector(".footer-subscription"),N=f.querySelector(".input-footer"),P=document.querySelector(".footer-button"),_=async e=>{try{return(await d.post("https://energyflow.b.goit.study/api/subscription",{email:e})).data}catch(t){throw new Error(t.message)}},F=async e=>{e.preventDefault(),P.blur();const t=N.value.trim();if(!D.test(t))w("Enter a valid email!");else{try{await _(t),R("Successful subscription!")}catch{w("There is already a user with this email!")}localStorage.removeItem(q),f.reset()}};f.addEventListener("submit",F);f.addEventListener("input",e=>{const t=N.value.trim(),n=JSON.stringify({footerEmail:t});localStorage.setItem(q,n)});d.defaults.baseURL="https://energyflow.b.goit.study/api";const z=document.querySelector(".exersizes-cards-container"),o=document.querySelector(".exersizes-cards-container-wrapper"),E=document.querySelector(".exersizes-result-card-container"),p=document.querySelector(".exersizes-result-card-container-wrapper"),l=document.querySelector(".exersizes-message-container"),U=document.querySelector(".exersizes-pagination-container"),C=document.querySelector(".exersizes-input-container"),a=document.querySelector(".exersizes-input-btn"),j=document.querySelector(".exersizes-input-btn-s"),I=document.querySelector(".dash"),g=document.querySelector(".exercise-name"),T=1,J=document.querySelector(".exersizes-list");document.querySelector(".exersizes-pagination-list");document.querySelector(".exersizes-pagination-btn");document.addEventListener("scroll",te);document.addEventListener("DOMContentLoaded",B());J.addEventListener("click",G);async function G(e){if(e.preventDefault(),e.target.nodeName==="BUTTON")try{h();const t=e.target.textContent.trim();sessionStorage.clear(),sessionStorage.setItem("filterType",JSON.stringify(t)),B(t),M(),K(),ie(),Q(e)}catch(t){console.log(t),c()}finally{v()}}o.addEventListener("click",W);async function W(e){if(e.preventDefault(),!(e.target.nodeName!=="DIV"&&e.target.nodeName!=="H3"&&e.target.nodeName!=="P")){const t=e.target.dataset.filter,n=e.target.dataset.target;try{h(),O(t,n,T),Z(e),se(),sessionStorage.setItem("filterSubType",JSON.stringify(n)),sessionStorage.setItem("filterType",JSON.stringify(t))}catch{c()}finally{v()}}}U.addEventListener("click",V);async function V(e){if(e.preventDefault(),e.target.nodeName==="BUTTON")try{const t=JSON.parse(sessionStorage.getItem("filterType")),n=e.target.textContent.trim();let s;h(),sessionStorage.getItem("filterSubType")&&(s=JSON.parse(sessionStorage.getItem("filterSubType"))),M(),Y(t,s,n),ee(e)}catch{c()}finally{v()}}async function B(e,t,n){const s=await d.get("/filters",{params:y(e,t,n)});try{if(s.data.results.length===0)throw new Error("No results found...");e=s.data.results[0].filter,$(s),b(s)}catch{c()}}async function O(e,t,n){const s=await d.get("/exercises",{params:y(e,t,n)});try{if(s.data.results.length===0)throw new Error("No results found...");A(s),n===1&&b(s)}catch{c()}}async function X(e,t,n){const s=await d.get("/filters",{params:y(e,t,n)});try{if(s.data.results.length===0)throw new Error("No results found...");e=s.data.results[0].filter,$(s)}catch{c()}}async function Y(e,t,n){t?await O(e,t,n):await X(e,t,n)}async function x(e){if(e.target.nodeName!=="BUTTON"&&e.keyCode!==13)return;const t=document.querySelector(".exersizes-input").value.trim().toLowerCase(),n=JSON.parse(sessionStorage.getItem("filterType")),s=JSON.parse(sessionStorage.getItem("filterSubType"));try{if(h(),t.length!==0){const r=await d.get("/exercises",{params:y(n,s,T,t)});if(r.data.results.length===0)throw new Error("No results found...");A(r),b(r)}}catch{c()}finally{u(),v()}}async function $(e){const t=e.data.results;o.classList.remove("hidden"),p.classList.add("hidden"),l.classList.add("hidden"),z.innerHTML="";const n=t.map(s=>{const r=s.filter,i=s.name;return`<li class="first-filter"><div class="exersizes-card-bytype" tabindex="0" style="background: linear-gradient(
    0deg,
    rgba(16, 16, 16, 0.7) 0%,
